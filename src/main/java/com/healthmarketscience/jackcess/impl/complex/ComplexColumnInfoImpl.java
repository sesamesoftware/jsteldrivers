  ...
  implements ComplexColumnInfo<V>
{
  ...
  public void addValue(V value) throws IOException {
  ...
  public void updateValue(V value) throws IOException {
  ...
  public void deleteAllValues(int complexValueFk) throws IOException {
    Iterator<Row> entryIter =
      getComplexValFkIter(complexValueFk, Collections.emptySet());
    try {
      while(entryIter.hasNext()) {
        entryIter.next();
        entryIter.remove();
  ...
  private static final class ComplexValueIdImpl extends ComplexValue.Id
  {
    ...
    private ComplexValueIdImpl(int value, RowId rowId) {
      _value = value;
      _rowId = rowId;
    }
